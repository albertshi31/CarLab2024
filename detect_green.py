...
signal.signal(signal.SIGINT, cleanup)  # Ctrl+C termination
signal.signal(signal.SIGTERM, cleanup) # Process kill signal
    ...
    if count > 0:
        for index in range(count):
            ...
            if block.m_signature == green_signature:  # Check for green laser
                ...
                # Move the servo to the calculated angle
                print(f"Moving servo to angle: {angle}")
                pwm_servo.ChangeDutyCycle((angle / 18) + 2.5)  # Convert angle to PWM duty cycle
                time.sleep(0.1)  # Small delay for smooth servo movement
...
# The program will automatically call cleanup() when terminated with Ctrl+C or a kill signal
